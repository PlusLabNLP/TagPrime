...
class Info:
    def __init__(self):
        ...
        self.role = Counter()
        self.role_num = 0
        self.entity_width = Counter()
        self.trigger_width = Counter()
    ...
    def update(self, instance):
        ...
        for entity in instance['entity_mentions']:
            ...
            self.entity_num += 1
            ent_wid = entity['end'] - entity['start']
            self.entity_width[ent_wid] += 1
        
        for event in instance['event_mentions']:
            self.event[event['event_type']] += 1
            self.event_num += 1
            tri_wid = event['trigger']['end'] - event['trigger']['start']
            self.trigger_width[tri_wid] += 1
    ...
    def __add__(self, obj):
        ...
        self.entity_width = self.entity_width + obj.entity_width
        self.trigger_width = self.trigger_width + obj.trigger_width
        return self
...
all_entity_width = Counter()
all_trigger_width = Counter()
for v in doc_statistics.values():
    all_entity_width += v.entity_width
    all_trigger_width += v.trigger_width
