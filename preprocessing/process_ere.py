...
def ere_to_oneie(input_file: str,
    ...
    with open(input_file, 'r', encoding='utf-8') as r, \
        ...
        for line in r:
            ...
            # tokens
            tokens = inst['tokens']
            pieces = [tokenizer.tokenize(t, is_split_into_words=True) for t in tokens]
...
def parse_arguments() -> Namespace:
    ...
    parser.add_argument('-l', '--lang', choices=['chinese', 'spanish', 'english'])
...
def main():
    ...
    if model_name.startswith('bert-'):
        ...
        tokenizer = RobertaTokenizer.from_pretrained(model_name,
                                                     cache_dir=cache_dir, add_prefix_space=True)
    ...
    if args.lang == 'english':
        ...
        sub_set = [
            ("LDC2015E107_DEFT_Rich_ERE_Spanish_Annotation_V2/data/", "spanish")
        ]

        for d_path, dataset in sub_set:
            # Convert to JSON format
            print('collecting {}........'.format(d_path))
            json_path = os.path.join(args.output, '{}.{}.json'.format("spanish",
                                                                    f_size))
            process_batch(os.path.join(args.input, d_path),
                        json_path,
                        window_size=args.window,
            ...
            # Convert to OneIE foramt
            oneie_path = os.path.join(args.output, '{}.{}.oneie.json'.format("spanish",
                                                                            f_size))
            ere_to_oneie(json_path, oneie_path, tokenizer=tokenizer)
        ...
        if args.split:
            all_data = []
            for ds in ["spanish"]:
                with open(os.path.join(args.output,'{}.{}.oneie.json'.format("spanish",f_size)), 'r', encoding='utf-8') as r:
                    for line in r:
                        all_data.append(json.loads(line))
            train_docs, dev_docs, test_docs = set(), set(), set()
            # load doc ids
            ...
            with open(os.path.join(args.output, 'train.json'), 'w') as w_train, \
                open(os.path.join(args.output, 'dev.json'), 'w') as w_dev, \
                open(os.path.join(args.output, 'test.json'), 'w') as w_test:
                for inst in all_data:
                    doc_id = inst['doc_id']
                    if doc_id in train_docs:
                        w_train.write(json.dumps(inst) + '\n')
                        ...
                        print('missing in the split file!! {}'.format(doc_id))
